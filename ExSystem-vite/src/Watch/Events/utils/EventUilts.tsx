...
export const EventVehicle = ({ vehicle }: { vehicle: Vehicle }) => {
    const boloHandler = () => {
        console.log("bolo vehicle", vehicle)
    }
        ...
        <Popover
            ...
            content={
                <Flex vertical gap={"middle"}>
                    ...
                    <Typography.Link className="ex-line ex-text event-message"
                        onClick={boloHandler}
                    >{"Bolo车牌信息"}</Typography.Link>
                </Flex>
            }>
...
export const EventPerson = ({ person }: { person: CIV }) => {
    const detailHandler = () => {
        console.log("detail person", person)
    }
        ...
        <Popover
            ...
            content={
                <Flex vertical gap={"small"}>
                    <Flex gap={"small"}>
                        <InfoItem title="名字" content={person.name} />
                        <InfoItem title="性别" content={CIV.GenderStringMap[person.gender]} />
                        <InfoItem title="族裔" content={CIV.RaceStringMap[person.race]} />
                        <InfoItem title="年龄" content={CIV.birthdayToAge(person.birthday)} />
                    </Flex>
                </Flex>
            }>
            <Button type={"text"} style={{ padding: "4px" }}
...
const InfoItem = ({ title, content }: { title: string, content: string | number }) => {
    return (
        <Flex>
