...
export const getMenuItem = (component: ExMenuComponents, active: boolean) => {
  switch (component.type) {
    ...
    case "options":
      return (
        <MenuOptions component={component as ExCOptions} active={active} />
      );
    case "input":
...
export const MenuPageComponent = ({
  ...
  useEffect(() => {
    if (active) context.enterHandler = () => context.render(page);
    return () => {
      context.reset();
    };
  }, [page]);
      ...
      <Flex className="my-auto ml-auto">
        <Flex className="ex-color my-auto" style={{ fill: "var(--gray-700)" }}>
          <svg height="18" viewBox="0 0 18 18" width="18">
            ...
            <path
              className="fill"
              d="M15,9a.994.994,0,0,1-.2925.7045l-3.9915,3.99a1,1,0,1,1-1.4355-1.386l.0245-.0245L12.5905,9,9.3045,5.715A1,1,0,0,1,10.691,4.28l.0245.0245,3.9915,3.99A.994.994,0,0,1,15,9Z"
            />
            <path
              className="fill"
              d="M9,9a.994.994,0,0,1-.2925.7045l-3.9915,3.99a1,1,0,1,1-1.436-1.385l.0245-.0245L6.5905,9,3.3045,5.715A1,1,0,0,1,4.6915,4.28l.0245.0245,3.9915,3.99A.994.994,0,0,1,9,9Z"
            />
          </svg>
        </Flex>
      </Flex>
...
export const MenuButton = ({
  ...
  useEffect(() => {
    if (active) context.enterHandler = component.enterHandler;
    return () => {
      context.reset();
    };
  }, [active]);
  ...
  useEffect(() => {
    if (active)
      context.enterHandler = () => {
        ...
        if (component.enterHandler) component.enterHandler(component.value);
  ...
  return (
    <Flex className="flex-grow">
      <Flex className="my-auto ml-auto" gap={"small"}>
        ...
        <Typography.Text>{component.value}</Typography.Text>
        <Flex className="ex-color my-auto" style={{ fill: "var(--gray-700)" }}>
          <svg height="18" viewBox="0 0 18 18" width="18">
            <rect
              id="Canvas"
  ...
  useEffect(() => {
    if (active) {
      ...
      context.leftHandler = () => {
        ...
        if (component.changeHandler) component.changeHandler(component.value);
        setState(component.value);
      };
      context.rightHandler = () => {
        const options = component.options;
